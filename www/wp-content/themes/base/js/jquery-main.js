...
function initPhotos(){
	...
	$fancyGallery.attr('class', 'product-thumbs-fancy');
	$fancyGallery.hide();
	...
	$('#product-thumbs a').click(function(e){
		...
		if(! $clicked.hasClass('active')){//Only fire the event on non-active images
			...
			$('#foto-image img').fadeOut('400',function(){//Fade out the old image
				$('#foto-image img').remove();
				...
				$('#foto-image').attr('href', $clicked.attr('href'));
				$('#foto-image').append($newImage);

				$('#foto-image img').imagesLoaded( function(){//Once image is loaded, bring it back in
					$('#foto-image img').fadeIn('400');
				});
			});
...
function initPhotoGallery() {
	jQuery(".photo-carousel").each(function(){
		...
		currentItem.find('.slideset').bxSlider({
			auto: false,
			autoDelay: 1000,
			speed: 1000,
			pause: 6000,
...
function initDatepicker() {
	$.datepicker.regional['it'] = {
		...
		nextText: 'Succ&#x3e;',
		currentText: 'Oggi',
		monthNames: ['январь','февраль','март','апрель','март','июнь',
